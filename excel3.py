...
#     if item.get('Asset type') == 'NON_STANDARD_INFOMATORY_SIGNS' or item.get('Anomaly type') == 'NON_STANDARD_INFOMATORY_SIGNS' or item.get('Asset type') == 'ADVERTISEMENT_ENCHROACHMENT_SIGNS'  or item.get ('Asset type') == 'DAMAGED_SIGN' or item.get ('Asset type') == 'DAMAGED_SIGNS':
#         return counts
...
#         if item['Asset type'] not in counts[side_key]:
#             counts[side_key][item['Asset type']] = 0
#         counts[side_key][item['Asset type']] += 1

#     elif 'Anomaly type' in item:
#         if item['Anomaly type'] not in counts[side_key]:
#             counts[side_key][item['Anomaly type']] = 0
#         counts[side_key][item['Anomaly type']] += 1
...
# #     if asset_type and asset_type != 'ADVERTISEMENT_ENCHROACHMENT_SIGNS':
# #         counts[side_key][asset_type] += 1
...
#         #         # Early exit condition to optimize loop
#         #         if distance >= distance_range[1]:
                    
#         #             continue  # Skip to the next item without processing further

#         #         if distance_range[0] <= distance < distance_range[1]:
#         #             side = item.get('Side', 'center').title()
#         #             counts = process_item(counts, item, side)
#         #             # print(distance, "distance", item, "item value")

#         #             # Define result key
#         #             key = distance_range[0]
#         #             result[key] = counts
                    
#         #             # Add GPS data
#         #             add_gps_data(result, key, gpx_data, distance_range[0], distance_range[1])
            
            
#         #     # except Exception as e:
#         #     #     print(f"Skipping this item due to an error: {e}")
#         #     #     continue
...
#         #     last_key = distance_range[1]
#         #     result[last_key] = result[key].copy()
#         #     add_gps_data(result, last_key, gpx_data, distance_range[1], distance_range[1])
...
# 2nd code of excel - 3 this code is not using MCW for both.  
